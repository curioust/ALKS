package com.coxautoinc.cia.alks.controller;
...
import com.coxautoinc.cia.alks.model.ui.User;
...
public class LoginController {
	...
	public static String ADMIN_AD_GROUP = MessageUtils.getMessage("ldap.service.account.adminGroup");
	...
	public ModelAndView loginPage(HttpServletRequest request){
		...
		request.setAttribute("warning", "As of May 11th 2016 this tool only accepts network id/user id instead of email");
	...
	public ModelAndView validate(@ModelAttribute(value="user") User user, ModelMap modelMap, HttpServletRequest request){
		...
		if(groups==null){
			...
			mav.addObject(MessageUtils.ERROR, MessageUtils.getMessage("com.coxautoinc.cia.alks.controller.LoginController.login.inValid") );
			logger.error(MessageUtils.getMessage("com.coxautoinc.cia.alks.controller.LoginController.login.inValid"));
		...
		//Display message to admin user if the AD groups don't match.
		if(user.isAdmin()){
			if(!ADGroupsServiceImpl.areADGropsSame()){
				mav.addObject("warning","AD Groups in Autotrader and Manheim are different.");
			}
		}
	...
	public @ResponseBody String getAccountId(@RequestParam(value = "selectedAccount", required = true) String accountNumber,
			@RequestParam(value = "selectedRole", required = true) String role, @RequestParam(value = "time", required = true) String time, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		String userName = sessionUser.getEmailId();
		if(userName.indexOf('@')>0){
			userName = userName.substring(0, sessionUser.getEmailId().indexOf('@'));
		}
		...
		if(keys.length>1){
			...
			response.sendError(HttpServletResponse.SC_UNAUTHORIZED,
                    "This account is not authorized, please contact system admin.");
		}
